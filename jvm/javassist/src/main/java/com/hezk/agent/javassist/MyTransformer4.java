...
public class MyTransformer4 implements ClassFileTransformer {

    public byte[] transform(ClassLoader loader, String className, Class<?> classBeingRedefined,
                            ProtectionDomain protectionDomain, byte[] classfileBuffer) throws IllegalClassFormatException {
        className = className.replace("/", ".");
        if ("sun.net.www.protocol.http.HttpURLConnection".equalsIgnoreCase(className)) {
...
//                bodyStr.append("headerValue = headerValue == null ? \"\" : headerValue;");
                bodyStr.append("this.setRequestProperty(\"l5d-ctx-dtab\", \"headerValue\");\n");
